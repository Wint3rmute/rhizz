...
## Task <N> — One view per file: compiler rework with per-file view validation

Complete rework, no backwards compatibility. The spec now says (`SPEC.md` §1,
`SPEC/models.md` ## Merge): multiple views live as individual files under
`diagrams/*.hcl`, exactly one `view` block per file, label matching the
filename. The compiler must enforce this instead of merging all views into a
single structure.

Background: `rhizz-core::compile()` currently merges `view` blocks from every
`.hcl` file into one `RawFile` and `resolve_view` (`crates/rhizz-core/src/resolve.rs`)
only checks `E006` (undefined system), silently dropping `node` layout blocks
(`crates/rhizz-core/src/parse.rs::parse_view`). `rhizz-book::compile_project`
(`crates/rhizz-book/src/project.rs`) filters `diagrams/` out entirely, so view
errors are invisible. `rhizz fmt` writes a root `views.hcl` holding many views.

Requirements:

1. **New error `E016` (blocking) with spec page `SPEC/diagnostics/E016.md`** —
   emitted per offending view file, with the file's relative path present in
   the error message (e.g. `diagrams/overview.hcl`). Fires when a single file
   under `diagrams/` contains zero or 2+ `view` blocks, or when the view label
does not match the filename stem (`diagrams/overview.hcl` must hold
   `view "overview"`). Assert the exact code (not just counts) in tests.
2. **Two-phase compilation, no view merging** — phase 1 compiles the system
   model (`system.hcl`/`main.hcl`) exactly as today; if it has errors, skip
   phase 2. Phase 2 validates each `diagrams/*.hcl` file independently against
   the already-resolved `Model` via a new `validate_view(&Model,
   &ViewDefinition, filename) -> Vec<Diagnostic>` helper. Initial checks only:
   one block (E016), label==filename (E016), `system` exists (reuse E006).
   `node`-path validation slots into the same helper later — keep the
   signature extensible but out of scope for this task. Never merge views into
   a single structure (no dedup-by-label); each diagram file must round-trip
   through `parse_views`/`serialize_views` independently, and `system.hcl`
   through `serialize_model`.
   End state (do not bolt E016 onto the merge — remove it): rip view handling
   out of `compile()` entirely — `RawFile.views`, `parse_view`, and
   `resolve_view` go away and `Model.views` is removed; views are parsed only
   via `serialize.rs::parse_views` and validated only via `validate_view`.
   `serialize_resolved_views` and its dedup-by-label hack go away with the
   merge.
3. **`rhizz-book::compile_project` uses the new flow** — stop excluding
   `diagrams/`; run phase 1 on model sources, then the per-file phase-2 loop,
   returning model + view diagnostics as one combined verdict (per-file
   isolation: one bad view file must not hide the others).
4. **`rhizz fmt` ignores views for now** — rewrite only `system.hcl`; do not
   read, write, or reformat any view file.
5. **No back-compat for `views.hcl`.** The compiler gives a root-level
   `views.hcl` the same E016 treatment as any other multi-view file (which
   forces the move — it cannot pass validation as-is). Migration rule: if a
   `views.hcl` sits under `diagrams/`, validate it as one-view-per-file; if it
   sits in the project root, move it under `diagrams/` (split into one file
   per view, see 6) and validate.
6. **Migrate `examples/` after implementation** — delete multi-view
   `views.hcl` files, split each view into `diagrams/<label>.hcl`, fix the
   duplicated `"main"` labels (drone, apollo-11, software-house), until every
   example compiles clean under the new rules.

Definition of done:

- E016 fires (with file path in message) for 0-view, 2+-view, and
  label-mismatch diagram files; E006 still fires for unknown `system`.
- `2_example_error`-style bad view references are now caught instead of
  silently dropped.
- All examples compile; `just test`, `just lint`, `just build` green;
  `just format` applied. Red/green TDD per repo workflow.
- Fallout surface (all in scope): WASM `parse_views`/`CompileResultJS` shape
  and wrapper tests, web `DocumentStore`/wrapper tests feeding
  `diagrams/main.hcl` through `compile`, CLI `load_sources` (must stop
  merging view blocks — model sources only), and `book/book.lock` (verdicts
  will churn once `diagrams/` is validated — regenerate with `just
  book-accept` and review the per-block diff before committing).
