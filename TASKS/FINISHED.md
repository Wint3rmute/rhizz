...
- **Dead code**: `diagramTitle` (only the breadcrumb's title) removed from
  `explore/navigation.ts` with its unit test.
...
Deliverable: `audit/architecture.md` (13 findings with impact/confidence,
locations, evidence and simplifications, plus a ranked "Highest-value
findings" list). No source, test, config or project files were modified.

Original task description (kept for reference):
