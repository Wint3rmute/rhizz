<script lang="ts">
  ...
  let overallPct = $derived(
    score ? Math.round(score.overall_percentage) : 0,
  );
    ...
    >
      ...
      <main class="md:col-span-6 lg:col-span-8 flex flex-col gap-6">
        ...
        {:else}
            ...
            <div class="card bg-gray-800 shadow">
              ...
              >
                <div>
                  ...
                  {#if project.version}
                    <span class="text-sm text-gray-400">v{
                        project.version
                      }</span>
                  {/if}
                </div>
                {#if project.authors.length > 0}
                  <div class="ml-auto flex gap-2 flex-wrap">
                    {#each project.authors as author}
                      <div class="badge badge-outline badge-sm text-gray-300">
                        {author}
                      </div>
                    {/each}
                  </div>
          ...
          <!-- ── Stats row ── -->
          <div
            class="stats stats-vertical sm:stats-horizontal shadow bg-gray-800 w-full"
          >
            <div class="stat">
            ...
            <div class="stat">
              ...
              <div class="stat-desc text-gray-500">
                {catPct(score?.ports ?? null)}% defined
              </div>
            </div>
            ...
            <div class="stat">
              ...
              <div class="stat-desc text-gray-500">
                {catPct(score?.connections ?? null)}% defined
              </div>
            </div>
            ...
            <div class="stat">
              ...
              <div class="stat-desc text-gray-500">
                {totalMessages} messages defined
              </div>
            </div>
          </div>
          ...
          <div class="card bg-gray-800 shadow">
            <div class="card-body">
              <h2 class="card-title text-gray-100 mb-6">
                Completion Breakdown
              </h2>
                ...
                {#each                 [
                  ...
                  <div class="bg-gray-900 rounded-xl p-4 flex flex-col gap-3">
                    <div class="flex items-center justify-between">
                      ...
                      <span
                        class="text-lg font-bold tabular-nums {pct >= 80 ? 'text-success' : pct >= 50 ? 'text-warning' : 'text-error'}"
                      >{pct}%</span>
                    </div>
                    <progress
                    ...
                    <div class="flex gap-2 text-xs flex-wrap">
                      ...
                      {:else}
                        ...
                        >{row.cat?.complete ?? 0} complete</span>
                        <span
                          class="badge badge-sm bg-yellow-900 text-yellow-300 border-0"
                        >{row.cat?.partial ?? 0} partial</span>
                        <span class="badge badge-sm badge-ghost text-gray-500">{
                            row.cat?.incomplete ?? 0
                          } incomplete</span>
                      {/if}
                    </div>
                  </div>
                    ...
                    <tbody>
                      {#each components as comp}
                        <tr class="border-gray-700 hover:bg-gray-700">
                          <td class="font-mono text-primary text-sm">
                            {comp.label}
                          </td>
                          <td>
                            <span
                              class="badge badge-sm {levelBadge(comp.level)}"
                            >L{comp.level}</span>
                          </td>
                          <td>
                            {#if comp.leaf}
