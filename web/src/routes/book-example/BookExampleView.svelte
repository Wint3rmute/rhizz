...
let errorDiags = $derived(diagnostics.filter((d) => d.level === "Error"));
let warningDiags = $derived(diagnostics.filter((d) => d.level !== "Error"));

function plural(count: number, word: string): string {
  return `${String(count)} ${word}${count === 1 ? "" : "s"}`;
}

// Classic book verdict panel content: status, head line, split diagnostics.
let verdictStatus = $derived.by((): "ok" | "warn" | "error" => {
  if (compileCrashed || errorDiags.length > 0) return "error";
  if (warningDiags.length > 0) return "warn";
  return "ok";
});
let verdictHead = $derived.by((): string => {
  if (compileCrashed) return "The project failed to compile in the browser.";
  if (errorDiags.length > 0) {
    return `${plural(errorDiags.length, "error")}, ${
      plural(warningDiags.length, "warning")
    } — no score (compilation failed)`;
  }
  if (warningDiags.length > 0) {
    ...
    return `${plural(warningDiags.length, "warning")} — ${tail}`;
  }
...
let stats = $derived.by((): VerdictStats | null => {
  const report = score;
