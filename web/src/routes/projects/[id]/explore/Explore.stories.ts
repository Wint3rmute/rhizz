...
// --- "Viewer story": the main seeded project, plus a doc file for the sensor
// node so the hover popup has content to show. Docs are matched by the
// component's unique label, so the doc key is the label ("sensor"), not the
// full qualified path.
const SENSOR_DOC =
  `# Sensor\n\nThe **environmental sensor** reads temperature and humidity over I2C.`;

function ensureSeededProject() {
  return ensureStoryProject({
    id: SEEDED_PROJECT_ID,
    name: "Viewer story",
    hcl: EXAMPLE_SYSTEM_HCL,
    files: [{ path: "docs/sensor.md", content: SENSOR_DOC }],
    diagrams: EXAMPLE_SYSTEM_DIAGRAMS,
  });
}

function ensureManyDiagramsProject() {
  return ensureStoryProject({
    id: MANY_DIAGRAMS_PROJECT_ID,
    name: "Many diagrams story",
    hcl: EXAMPLE_SYSTEM_HCL,
    diagrams: manyDiagramsMap,
  });
}

function ensureCrossLevelProject() {
  return ensureStoryProject({
    id: CROSS_LEVEL_PROJECT_ID,
    name: "Cross level connections story",
    hcl: CROSS_LEVEL_SYSTEM_HCL,
    diagrams: CROSS_LEVEL_SYSTEM_DIAGRAMS,
  });
}

function seedApolloProject() {
  return seedExampleProject(APOLLO_PROJECT_ID, "Apollo 11 story", "apollo-11");
}

function seedSoftwareHouseProject() {
  return seedExampleProject(
